...
发布行级评论时，建议使用以下统一格式，并确保“分类”使用速查表中的代码标签（如 `[REQ]`、`[LOGI]`、`[SEC]`）：
...
模块：args
等级：Major
问题：文档声明支持 JSON 数组参数，但当前分支仅解析 JSON 对象，导致行为与文档不一致。
原因：用户按文档传参会触发解析失败，影响参数协议兼容性与可用性。
修改意见：补齐 JSON 数组解析路径，并增加对象/数组双形态回归测试；同步更新示例与说明。
```
